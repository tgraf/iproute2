...
static void print_encap_bpf_prog(FILE *fp, struct rtattr *encap, const char *str)
{
...
static void bpf_lwt_cb(void *_x, int fd, const char *annotation)
{
	struct lwt_x *x = _x;
...
static void lwt_bpf_usage(void)
{
	...
	fprintf(stderr, "BPF := obj FILE [ section NAME ] [ export FILE ] [ verbose ]\n");
	exit(-1);
}
			   ...
			   char ***argvp)
{
	...
	while (argc > 0) {
		if (strcmp(*argv, "in") == 0) {
			...
			if (!headroom_set)
				rta_addattr32(rta, 1024, LWT_BPF_XMIT_HEADROOM, headroom);
			headroom_set = 1;
